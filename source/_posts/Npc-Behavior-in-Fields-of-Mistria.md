...
This response is long overdue, but your question requires a long answer.
...
My intended audience is, ideally, a designer/programmer who is interested in implementing NPCs in a farming sim, town simulator, or other slice-of-life style game. A little bit of design and programming knowledge will be useful, particularly around pathfinding algorithms, but I hope that I'll explain enough that a newcomer in this space won't be lost. They say all advice is fundamentally autobiographical, and this article is no exception: if you just want to learn about some of *Mistria's* development, I hope this article will be entertaining enough.
...
1. NPCs must animate, make noises, and cause other feedback in certain positions.
...
Our NPCs, then, should be very simple to program, right? Presumably, animating a sprite, creating audio, or any other feedback is handled in your technical setup already, so all we need to do is figure out how to move between positions at a particular time, and we're good to go. Moving between points, in this case, would mean playing a walking animation and then moving their location at some slow rate, simulating movement. Most of this article is going to be on how and why to do this.
...
> *Fields of Mistria* only allows the player to place furniture in their house or on the Farm. We actually *do* have a few cutscenes on the Farm, but they only happen in the early game, when players are unlikely to have placed any furniture. The worst offender here, though, is "The Unusual Statue, Part 1", which is a cutscene that takes place on the start of Day 2 -- at this point, it *is* possible for players to get furniture and place it on the farm (though relatively unlikely). As we'll see in this article, we design our NPCs to algorithmically walk between points, but if some collision is already *on the point they're trying to get to*, the game can fail to pathfind them correctly. We tend to choose to crash (fail an assertion) in this case during cutscenes as a form of error reporting, since it will tell us if a cutscene was set up incorrectly. To prevent the player from crashing the game, we actually "poof" all the furniture within a certain area (ie, we convert them back to placeable items) so the NPCs can walk around freely. However, were players to place a Barn, or other complex structure, at this point (which is impossible without cheating), there would be some chaotic results! We have later cutscenes, such as the beginning of each festival, that take place on the farm, but they only take place within a "safe zone", directly around the player's door, around Caldarus's statue, or right near the exits of the map, which are all places where the Player cannot place furniture at all.
...
Finally, even if we could regenerate paths *and* somehow stop appropriately to provide feedback for players, we still would be breaking verisimilitude because the NPCs would be reasoning about the world *globally*, not *locally*. Roughly speaking, the NPCs should only react to something within about 300 pixels. This, of course, is about the radius of a circle fit to the Camera's frustum, which makes the NPCs just like the Player!
...
There is one final problem we haven't addressed though: NPC-on-NPC *moving* collisions (where two NPCs are basically walking into each other and neither is static) could generate infinite waiting games. These waiting games were a funny recreation of something that happens in real life ("After you", "No, after you", "No, after you", ad infinitum) but created traffic pileups in commonly seen areas. We solved this by creating an NPC priority order (which is simply just their alphabetical order, which is really just the order of an `enum NpcId`). Effectively, when Adeline, who is the first NPC by alphabetical order, is moving and the meddler is also moving, Adeline will *not* patiently wait for the Meddler and instead will immediately query a new path if the Meddler's priority is lower than Adeline. Adeline (with an "A" name) has the highest priority of any NPC, so generally wins in these comparisons, but, as you'd expect, the Player has the truly highest priority, so Adeline will give way to them. Ultimately, this priority game is what actually makes NPCs wait for the player!
...
> We went down many rabbitholes early in development to try to make pathfinding "more interesting" for certain NPCs. For example, Dell, one of the children in the game, would *prefer* paths with a higher cost, reflecting her "off-roading" nature. However, in practice, rather than just walk next to paths, like a kid who likes playing in the dirt like we wanted her to, she mostly just took very circuitious routes that confused us as we watched. Even more damming, though, no one really noticed in playtesting that her behavior was weird unless it was pointed out directly. This problem will be explored in the next post in this series, but *legibility* of NPC behavior is a huge effort in *Mistria* -- players rarely notice an NPC's "weird" ambient behavior, since players are focused on themselves. Ultimately, it was simpler for everyone if NPCs minimized "cost" rather than some other metric.
...
Here's the kicker for steps 3 and 4: we can perform both of these at compile-time. Who doesn't love doing things at compile-time instead of runtime? That's because, essential for this technique, *Mistria* never adds more doors (or shortcuts) that NPCs can walk through and the player cannot place furniture in places where the NPCs travel through. If we did add those, then we'd need to do stage 3 and 4 at runtime instead of compile-time.
