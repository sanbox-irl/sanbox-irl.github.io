...
data is good enough. These named positions also include every door, so when we're done with this first step, which is ran at compile time, we can also pathfind *between every map door to every other map door.* This process is simpler than it appears, but ultimately we do it with A\*, but on the domain of *only doors*, rather than the grid itself.
